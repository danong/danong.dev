...
deletes its bookmark, fetches the merge commit, advances the local `main`
bookmark when that is a fast-forward, and returns the working copy to the merge
commit when it shipped the current working copy. It accepts an optional
revision, such as `./scripts/jj-ship @-`. It requires `jj`, `gh`, and GitHub CLI
authentication.
